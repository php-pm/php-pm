...
class StartCommand extends Command
{
    ...
    protected function configure()
    {
        ...
        $bridge        = $this->optionOrConfig($config, 'bridge', 'HttpKernel');
        $port          = (int) $this->optionOrConfig($config, 'port', 8080);
        $workers       = (int) $this->optionOrConfig($config, 'workers', 8);
        $appenv        = $this->optionOrConfig($config, 'app-env', 'dev');
        $appBootstrap  = $this->optionOrConfig($config, 'bootstrap', 'PHPPM\Bootstraps\Symfony');
    ...
    private function optionOrConfig($config, $name, $default) {
        $val = $default;
